...
기존 OpenGL 중심 구조의 한계를 극복하기 위해 멀티 백엔드 렌더링 엔진 라이브러리를 설계·개발했습니다.<br>
...
이를 기반으로 멀티 백엔드 렌더링 엔진 아키텍처를 설계·도입
- OpenGL Core Profile 기능의 완전 지원을 1차 목표로 OpenGLDriver를 구현
- 레거시 OpenGL 사용 코드를 Driver 인터페이스 기반 구조로 대체하는 리팩토링 수행
...
하나는 메인 렌더링을 담당하는 Context이며, 다른 하나는 리소스 생성·해제를 담당하는 서브 Context로 구성되어
